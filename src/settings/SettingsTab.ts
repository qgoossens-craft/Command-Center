...
export class CommandCenterSettingsTab extends PluginSettingTab {
    ...
    display(): void {
        ...
        new Setting(iconsEl)
            ...
            .addToggle(toggle => toggle
                ...
                .onChange(async (value) => {
                    ...
                    await this.plugin.saveSettings();
                    this.plugin.refreshHomepage();
                    this.display(); // Refresh to show/hide icon customization options
                }));
        ...
        if (this.plugin.settings.customIcons.useCustomIcons) {
            ...
            iconPickerDesc.createEl('p', { text: 'Search through 300+ icons and click to copy to clipboard:' });
            
            const iconPickerContainer = iconPickerDesc.createDiv({ cls: 'enhanced-icon-picker' });
            
            // Create the enhanced icon picker
            new IconPicker(iconPickerContainer, {
                onIconSelect: async (icon: string) => {
                    try {
                        await navigator.clipboard.writeText(icon);
                        new Notice(`Copied "${icon}" to clipboard! Paste it into any icon field above.`);
                    } catch (error) {
